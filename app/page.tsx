...
export default function Home() {
  ...
  const scale = useTransform(scrollYProgress, [startAnim, midAnim, 1], [1, 0.65, 0.65]);
  const y = useTransform(scrollYProgress, [startAnim, midAnim, 1], ["0vh", isMobile ? "28vh" : "12vh", isMobile ? "28vh" : "12vh"]);
  const borderRadius = useTransform(scrollYProgress, [startAnim, midAnim, 1], ["0px", isMobile ? "40px" : "50px", isMobile ? "40px" : "50px"]);
  const overlayOpacity = useTransform(scrollYProgress, [startAnim, isMobile ? 0.5 : 0.3, 1], [0, 0.9, 0.9]);

  const textStart = isMobile ? 0.35 : 0.05;
  const textMid = isMobile ? 0.55 : 0.35;
  const textOpacity = useTransform(scrollYProgress, [textStart, textMid, 1], [0, 1, 1]);
    ...
    <main id="top" className="relative bg-[#ffffff]">
      ...
      <div ref={containerRef} className="relative h-[400vh]">
        <div className="sticky top-0 min-h-screen w-full overflow-hidden flex flex-col justify-center">
          <motion.div
            style={{ scale, y, borderRadius, transformOrigin: "top center", backgroundImage: "url('/images/background.jpg')" }}
          ...
          >
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-12 pt-20 lg:pt-10 grid grid-cols-1 lg:grid-cols-2 items-center gap-4 sm:gap-8 lg:gap-12 flex-grow pb-16 lg:pb-0 lg:h-full">
              <div className="space-y-4 lg:space-y-6 flex flex-col items-center lg:items-start text-center lg:text-left">
                <p className="text-white/80 tracking-widest text-xs lg:text-sm font-medium uppercase italic">Ascend. Explore. Awaken.</p>
                <h1 className="text-white text-4xl lg:text-6xl leading-tight font-serif">Your Immersive Journey <br className="hidden lg:block" /> Through Sri Lanka <br className="hidden lg:block" /> Begins Here.</h1>
                <button className="group flex items-center gap-2 px-6 lg:px-8 py-3 rounded-full text-[#3A2E12] font-semibold text-sm tracking-wide hover:scale-105 transition-transform" style={{ background: "linear-gradient(135deg, #d4af37, #f2d388, #d4af37)", boxShadow: "0 4px 20px rgba(212, 175, 55, 0.4)" }}>
                  PLAN YOUR VOYAGE <ArrowUpRight className="w-4 h-4 lg:w-5 lg:h-5 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
                </button>
              </div>
              <div className="flex items-center justify-center w-full mt-4 lg:mt-0">
                <div className="flex flex-col items-center justify-center w-full max-w-[600px] lg:max-w-none">
                  ...
                  <p className="text-white/70 text-[10px] lg:text-sm tracking-[0.2em] font-light animate-pulse uppercase mt-0 -translate-y-2 lg:translate-y-0 lg:-mt-20 lg:translate-x-32">Click on dots in the map</p>
                </div>
              </div>
            </div>
            <motion.div style={{ opacity: overlayOpacity }} className="absolute inset-0 bg-[#0a0f16] z-40 pointer-events-none" />
            <motion.div style={{ opacity: textOpacity, scale: textScale }} className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none">
              ...
              >
                ...
                <div className="w-full lg:w-[60%] space-y-6 flex flex-col items-start text-left">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-[#d4af37]">{dest.id}</span>
                  </div>
