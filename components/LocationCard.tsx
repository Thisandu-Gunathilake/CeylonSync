...
export default function LocationCard({ data, onClose }: LocationCardProps) {
  return (
    <div className="relative w-72 rounded-3xl bg-slate-950/60 backdrop-blur-xl border border-white/10 p-5 shadow-2xl overflow-visible">
      ...
      <div className="relative w-full h-40 rounded-2xl overflow-hidden mb-5">
        <Image
          src={data.imageUrl} // Ensure you have /images/kandy-card.png in your public folder
          alt={data.name}
          fill
          priority
      ...
      <div className="space-y-2 text-left">
        <h2 className="text-white font-bold text-xl uppercase leading-tight font-serif tracking-tight">
          {data.name}: <span className="font-medium capitalize text-lg tracking-normal text-white/90 font-sans">{data.tagline}</span>
        </h2>
